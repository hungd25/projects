...
def predict_prob(classifiers, X):
    ...
    """
    avg_model = []
    for idx in classifiers:
        pred_mod = idx.predict_prob(X)
        avg_model.append(pred_mod)
    average_classifiers = sum(avg_model) / float(len(avg_model))
    return average_classifiers
