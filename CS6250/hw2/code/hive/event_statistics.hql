...
-- ***** your code below *****
FROM events
WHERE patient_id NOT IN (SELECT patient_id FROM mortality);
...
-- ***** your code below *****
FROM (SELECT count(event_id) AS event_count FROM alive_events
...
-- ***** your code below *****
FROM (SELECT patient_id, DATEDIFF(max(time), min(time)) as record_length FROM alive_events GROUP BY patient_id) record_length_alive;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT patient_id, DATEDIFF(max(time), min(time)) as record_length FROM dead_events GROUP BY patient_id) record_length_dead;
